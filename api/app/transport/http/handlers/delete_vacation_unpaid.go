...
func DeleteVacationUnpaid(c *core.Core, w http.ResponseWriter, r *http.Request) {
	w.Header().Add("Content-Type", "application/json")
	errorContainer := httperror.ErrorResponse{}

	message := domain.DeleteVacationUnpaidRequest{
		Token:            r.Header.Get("Authorization"),
		VacationUnpaidId: chi.URLParam(r, "vacation_unpaid_id"),
	}

	err := c.DeleteVacationUnpaid(r.Context(), &message)
	if err != nil {
		switch {
		...
		case errors.Is(err, core.ErrVacationIdNotFound):
			errorContainer.Done(w, http.StatusNotFound, "vacation unpaid id not found")
			return
		}
